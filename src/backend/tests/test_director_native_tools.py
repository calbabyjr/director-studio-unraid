...
def test_prop_design_tool_requires_identity_and_source_fields():
    parameters = PROP_DESIGN_TOOL["function"]["parameters"]

    assert parameters["required"] == ["name", "description", "generation_prompt"]
    assert "source_asset_id" in parameters["properties"]
    assert "image_index" in parameters["properties"]


def test_prop_design_does_not_inject_storyboarding_for_an_unplanned_script(
    tmp_projects_dir,
):
    project = create_project("Props before shots", "A detective enters.")

    tools, notes = sanitize_tools_for_pipeline(
        [{"name": "queue_prop_design", "args": {"name": "Red chair"}}],
        project=project,
        shots=[],
    )

    assert tools == [{"name": "queue_prop_design", "args": {"name": "Red chair"}}]
    assert notes == []
