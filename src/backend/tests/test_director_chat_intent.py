...
from app.agents.director.intent import (
    actor_design_intent,
...
def test_extract_chair_as_prop_offers_queue_prop_design(tmp_projects_dir):
    project = create_project("Testing 1.0", "A scene in a dungeon.")
    tools = _director_tool_schemas(
        project,
        current_message="Can the red stirrup chair be extracted out into a prop?",
    )
    assert [item["function"]["name"] for item in tools] == ["queue_prop_design"]
    assert prop_design_intent("extract the red stirrup chair into a standalone prop")
