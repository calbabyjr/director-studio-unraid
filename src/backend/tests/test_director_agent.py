...
def test_parse_shot_drafts_coerces_inventory_shaped_matches():
    from app.agents.director.planner import parse_shot_drafts

    raw = json.dumps(
        [
            {
                "scene_id": "scn_a92a9e2",
                "asset_matches": [
                    {
                        "id": "act_e24fb6616795",
                        "matches": [{"file_key": "master", "asset_filename": "Jenny.png"}],
                    }
                ],
                "voice_matches": [],
            }
    ...
    assert draft.title == "Shot 1"
    assert draft.shot_type == "medium shot"
    assert draft.asset_matches[0].asset_id == "act_e24fb6616795"
    assert draft.asset_matches[0].role == "actor"
    assert draft.asset_matches[0].file_key == "master"
    assert draft.asset_matches[0].picture_index == 1


def test_parse_shot_drafts_coerces_shot_label_and_actor_ids():
    from app.agents.director.planner import parse_shot_drafts

    raw = json.dumps(
        [
            {
                "shot": "15-26s: Jenny and Wendy enter the corridor",
                "actors": ["act_e24fb6616795", "act_bbbbbbbbbbbb"],
            }
        ]
    )
    draft = parse_shot_drafts(raw)[0]
    assert "corridor" in draft.script_beat
    assert [m.asset_id for m in draft.asset_matches] == [
        "act_e24fb6616795",
        "act_bbbbbbbbbbbb",
    ]
    assert [m.role for m in draft.asset_matches] == ["actor", "actor"]
