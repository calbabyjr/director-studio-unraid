...
from .visual_direction import analyze_ref_frame, fallback_visual_direction
from .planner import (
...
class DirectorService:
    ...
    async def queue_reference_frame(
        ...
        *,
        ...
        vision_model = model
        vision_picker = getattr(self.plan_provider, "vision_model", None)
        if callable(vision_picker):
            vision_model = await vision_picker()
        try:
            ...
            logger.exception(
                "ref_frame visual direction failed for %s; queueing Comfy with fallback prompt",
                shot.id,
            )
            ...
            _record_layout_generation_issue(
                shot, [f"visual direction failed, using fallback prompt: {exc}"]
            )
        ...
        job = create_job(
            ...
            params={
                ...
                "visual_director_model": vision_model,
