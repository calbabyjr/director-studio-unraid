...
def fallback_visual_direction(
    ...
    *,
    ...
    """Queue Comfy even if the vision LLM cannot inspect Pictures."""
    ordered = list(images.items())
    image_jobs = "; ".join(
        f"{caption} is a locked visual reference" for caption in captions
    ) or "use the attached stills as locked visual references"
    prompt = (
        f"{shot.shot_type or 'medium shot'}, {shot.camera_angle or 'eye-level'}, "
        f"{shot.camera_motion or 'locked-off'}. {shot.composition or ''} "
        f"{shot.script_beat or ''}. {image_jobs}. "
        "Keep identity, wardrobe, architecture, and lighting from the attached images."
    ).strip()
    brief = VisualBrief(
        shot_type=shot.shot_type or "medium shot",
        camera=f"{shot.camera_angle or 'eye-level'}; {shot.camera_motion or 'locked-off'}",
        scene_lock=[shot.composition or "keep the same location and lighting"],
        characters=[],
        forbidden=["character sheet", "multi-panel", "turnaround", "split screen"],
