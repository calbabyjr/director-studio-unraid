...
async def vram_status() -> dict:
    ...
    if provider is not None:
        ...
        if provider.provider_id == "ollama":
            ...
            # Plan-model lookup can be empty while a vision model is resident.
            try:
                live = await provider.client.loaded_models()
            except Exception:
                live = []
            if live:
                ollama_loaded = list(live)
    else:
    ...
    return {
        ...
        "resident_vram": sum(int(m.get("size_vram") or 0) for m in ollama_loaded),
        "ollama_on_gpu": ollama_vram > 0
        or any(int(m.get("size_vram") or 0) > 0 for m in ollama_loaded),
        "ollama_ps": [
            {
