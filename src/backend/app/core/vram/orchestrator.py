...
class VramOrchestrator:
    ...
    async def before_comfy_job(self, pipeline_id: str) -> None:
        ...
        if (self.policy or "exclusive") != "exclusive":
            logger.info(
                "shared VRAM policy: Comfy pipeline=%s will not unload the LLM",
                pipeline_id,
            )
            return
    ...
    async def after_comfy_job(self, pipeline_id: str, terminal_status: str) -> None:
        ...
        # Shared policy keeps Comfy weights on the 3090 between jobs.
        if (self.policy or "exclusive") == "exclusive":
            await self.release_comfy_models()
        else:
            logger.info(
                "shared VRAM policy: leaving Comfy models loaded after pipeline=%s",
                pipeline_id,
            )
        ...
        if (self.policy or "exclusive") != "exclusive":
            ...
            try:
                ...
                async with self._cv:
                    ...
                    self._cv.notify_all()
                    logger.info(
                        "GPU ownership released by llm (shared policy, ready=%s)",
                        self._llm_ready,
                    )
            return
